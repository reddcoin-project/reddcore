...
describe('Unit', function() {
  ...
  it('has property accesors "RDD", "mRDD", "uRDD", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'RDD');
    unit.RDD.should.equal(1.2);
    unit.mRDD.should.equal(1200);
    unit.uRDD.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromRDD('1.00001');
    unit.RDD.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mRDD.should.equal(1.00001);
    ...
    unit.RDD.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.RDD);
    Unit.RDD.should.equal('RDD');

    should.exist(Unit.mRDD);
    Unit.mRDD.should.equal('mRDD');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'RDD');
    unit.to(Unit.RDD).should.equal(unit.RDD);
    unit.to(Unit.mRDD).should.equal(unit.mRDD);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'RDD');
    unit.toRDD().should.equal(unit.RDD);
    unit.toMilis().should.equal(unit.mRDD);
    unit.toMillis().should.equal(unit.mRDD);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'RDD'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'RDD').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'RDD').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
